...
export default function Item(props) {
  ...
  // :point_right: STEP 7 - We need to pull item from items, using a parameter in the URL (:itemID)
  ...
  return (
    <div className="item-wrapper">
      ...
      <nav className="item-sub-nav">
        {/* :point_right: STEP 8 - Here go the NavLinks to `<current url>/shipping` and `<current url>/description` */}
        <NavLink to={`${url}/shipping`}>Shipping</NavLink>
        <NavLink to={`${url}/description`}>Description</NavLink>
      </nav>
      {/* :point_right: STEP 9 - Here go the Routes for `<current path>/shipping` and `<current path>/description` */}
      ...
      {/* :point_right: STEP 10 - Shorten paths and urls with `useRouteMatch` hook */}
    </div>
  );
}
